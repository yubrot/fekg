...
export default function Confirm({
  ...
  return (
    <div
      className={`fixed z-50 inset-0 bg-opacity-25 bg-bluegray-600 flex flex-col justify-around items-center ${transitionClasses[transition]}`}
    ...
    >
      ...
      >
        <div className="label p-6">{title}</div>
        <div className="px-6">{children}</div>
        <div className="mt-6 py-3 px-6 border-t border-bluegray-300 bg-bluegray-100 flex items-center space-x-4">
          <button
            className={`button flex-grow space-x-2 py-2 px-4 flex justify-center items-center rounded-sm hover:bg-bluegray-200 ${
              acceptClassName ?? ''
            }`}
